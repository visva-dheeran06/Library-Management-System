...
public class LibrarySystem {
    ...
    public String issueBook(String bookId, String memberId) {
        ...
        if (book == null || member == null) return "ERR: Invalid book or member ID.";

        if (book.isAvailable()) {
            book.availableCopies--;
            book.timesBorrowed++;
            member.borrowedBooks.put(bookId, java.time.LocalDate.now());
            history.get(memberId).push(bookId + " - ISSUED - " + java.time.LocalDate.now());
            return "OK: '" + book.title + "' issued to " + member.name + ".";
        } else {
            if (!waitingQueues.get(bookId).contains(memberId)) {
                waitingQueues.get(bookId).enqueue(memberId, member.getPriority());
            }
            return "WAIT: No copies available. " + member.name
                    + " added to waiting list (priority: " + member.membershipType + ").";
        }
    }
    ...
    public String returnBook(String bookId, String memberId) {
        ...
        book.availableCopies++;
        history.get(memberId).push(bookId + " - RETURNED - " + java.time.LocalDate.now());

        String msg = "OK: '" + book.title + "' returned by " + member.name + ".";
        if (fine > 0) msg += " Overdue! Fine: Rs." + fine;
        ...
        if (waitQ != null && !waitQ.isEmpty()) {
            ...
            if (nextMember != null) {
                ...
                history.get(nextMemberId).push(bookId + " - ISSUED - " + java.time.LocalDate.now());
                msg += "\nAuto-issued to: " + nextMember.name
    ...
    public List<String> getHistory(String memberId) {
        ...
        return stack != null ? stack.toList() : new ArrayList<>();
    }
    ...
    public List<String[]> getWaitingList(String bookId) {
        ...
        for (String mid : q.toList()) {
            ...
            result.add(new String[]{mid, m.name, m.membershipType});
        }
        return result;
    }
}
